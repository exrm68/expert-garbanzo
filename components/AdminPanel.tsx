          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              ...
              <div className="bg-[#1a1a1d] rounded-[20px] p-4 space-y-3 border border-white/5">
                ...
                <div>
                  <label className="block text-sm text-zinc-400 mb-1">🤖 Bot Username</label>
                  <p className="text-xs text-zinc-500 mb-2">@ ছাড়া দাও — যেমন: <span className="text-amber-400 font-bold">CineFlix_bot</span></p>
                  <input type="text" value={botUsername} onChange={(e) => setBotUsername(e.target.value.replace('@', '').trim())} placeholder="CineFlix_bot" className="w-full px-4 py-3 bg-[#0d0d10] border border-white/5 rounded-[14px] text-white text-sm outline-none focus:border-amber-500/30 transition-colors placeholder:text-zinc-700" />
                </div>

                <div>
                  <label className="block text-sm text-zinc-400 mb-1">📱 Mini App Name <span className="text-zinc-600 text-xs">(Optional)</span></label>
                  <p className="text-xs text-zinc-500 mb-2">BotFather এ Mini App এর নাম কী দিয়েছ? — যেমন: <span className="text-amber-400 font-bold">app</span></p>
                  <p className="text-xs text-zinc-600 mb-2">খালি রাখলে link: <span className="text-blue-400">t.me/BOT?startapp=...</span> &nbsp;|&nbsp; দিলে link: <span className="text-blue-400">t.me/BOT/APPNAME?startapp=...</span></p>
                  <input type="text" value={appName} onChange={(e) => setAppName(e.target.value.replace('/', '').replace('@', '').trim())} placeholder="app (BotFather এ যা দিয়েছ)" className="w-full px-4 py-3 bg-[#0d0d10] border border-white/5 rounded-[14px] text-white text-sm outline-none focus:border-amber-500/30 transition-colors placeholder:text-zinc-700" />
                </div>
...
const WithdrawalsPanel: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-black text-white tracking-tight">💸 Withdrawal Requests</h2>
        {pendingCount > 0 && (
        ...
        {(['all', 'pending', 'success', 'cancelled'] as const).map(f => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-bold capitalize transition-all ${
              filter === f ? 'bg-red-600 text-white' : 'bg-[#242427] text-zinc-400 hover:text-white'
            }`}
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-10 text-zinc-500">কোনো request নেই</div>
      ) : (
        filtered.map((w: any) => (
          <div key={w.id} className="bg-[#242427] rounded-2xl p-5 border border-white/10">
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-zinc-300 text-sm font-medium">{w.method === 'bkash' ? 'bKash' : 'Nagad'}</span>
            ...
            {w.adminNote && (
              <div className="bg-white/10 rounded-xl px-3 py-2 mb-3">
                <p className="text-zinc-300 text-sm">📝 {w.adminNote}</p>
              </div>
            )}
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                ...
                className="px-4 bg-white/10 hover:bg-white/15 text-white py-2.5 rounded-xl text-sm font-bold transition-colors"
              >
                📝 Note
              </button>
            </div>
        ...
        {noteModal && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0.9 }}
              className="bg-[#1a1a1d] rounded-2xl p-6 w-full max-w-sm border border-white/10"
            >
              ...
              <textarea
                ...
                className="w-full bg-[#242427] border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none resize-none h-28 mb-4"
              />
              <div className="flex gap-3">
                <button onClick={() => setNoteModal(null)} className="flex-1 py-3 bg-white/10 text-white rounded-xl font-bold">Cancel</button>
